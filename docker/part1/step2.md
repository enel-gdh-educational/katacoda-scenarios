...
Every journey in IT starts with an Hellow world, so run:
...
The container's images are one of the most important element. Someone can confuse Docker containers and Docker images, so let's explain.

An **image** is model for the creation of one or more containers. It's a stastic element and it isn't executed directly.

A **container** is a running entity, created from an image. So, from an image, we can create more containers.

So Docker, for create our "hello world" container used the "hello-world" image. But where Docker found it? Answetr, in Docker Hub. It's a public registry that contains Docker images ready do be executed. Docker Hub it's the public registry of Docker, but there are also private registry. These public registries usually contains base images that can be used to package our applications. For example there are images for Ubuntu, PostgreSQL, MongoDB, Kafka, and Python.
...
Containers reamin until we delete them with `docker rm hello hello2`{{execute}} command. This command remove one or more container. You must pass the container name or the container ID.
