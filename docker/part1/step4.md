...
Let's assume that we want to build a simple microservice that exposes two endpoints:
- /health : to check the health of the service
- /predict : to make predictions using an AI model

To developing our app we will use FastAPI, a framework to build web applications in Python. Look at the `app/api.py` file. In this script, we define the FastAPI app and define the `/health` endpoint. This base app will be expanded in the next steps.

## Try it locally

Let's try this app locally.
Navigate to the step4 directory `cd /root/project/step4`{{execute}},

install dependencies with `pip install -r requirements.txt`{{execute}}

and run `uvicorn app.api:app --host 0.0.0.0 --port 80`{{execute}}

We started our app locally. Now, open a new Terminal window and test it. Try to run `curl http://localhost/health`{{execute}}

Does it work? Ok... Now imagine that you have to ship it to someone else. What should you do? At least, you must pass him the code, you must be sure he has the right Python version installed and that he installs all dependencies. There can be conflicts between different library versions and different Python versions. 
...
Let's package our application in a Docker image with all its dependencies and code.

## Dockerize it!

Inside the `step4` directory, create a Dockerfile. `touch Dockerfile`{{execute}}

As usual, start with the "FROM" instruction. 
In this exercise we could use alphine and install all requirements we need (python, FastAPI, uvicorn and other dependencies). 
Fortunately, there is an image called `python`, publicly available [here](https://hub.docker.com/_/python) on DockerHub - an image ready to execute Python code. 
So, insert `FROM python:3` in the Dockerfile. 

With the ":3" we are specifying the Python version. But it's not a magic process! Someone built Docker images with python3, python2, python2.7 and so on... So we can go to the [Python Docker image page](https://hub.docker.com/_/python) and choose the right version for our use-case between the versions available.

**NOTE:** It's not recommended to use `alpine` image for data science, since it's difficult to install `pandas` library.

### COPY Command

Now we need to provision the environment with dependencies. So we will copy the `requirements.txt` file in the container and use it to install dependencies. To copy files in a container we can use the `COPY` instruction.
...
The RUN instruction will execute any command in a new layer on top of the current image and will commit the results. The resulting committed image will be used for the next step in the Dockerfile. Commands specified in this format run in a shell, which by default is `/bin/sh -c` on Linux or `cmd /S /C` on Windows.
...
When we will build this image, Docker will put the requirements file in / path, it will update pip and it will install all dependencies.
...
We need to tell Docker that our app needs to **expose** the port 80, and we need also to tell Docker to link a port on our machine to the container port.
...
This is necessary because we want to communicate with the container, but Docker containers can communicate to each other also without binding their port to the host machine. In this case, the first step (EXPOSE) is always necessary.
...
Why doesn't it work? Because we run two distinct containers that listen on the same port, but we mapped them to the same host port! So, it's not a problem having two or more container that listen on the same port, because these are container's ports. 
